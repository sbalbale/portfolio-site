        ...
        <div className="flex flex-col justify-center w-full lg:w-1/2">
          ...
          <div className="flex flex-col gap-5 md:gap-6 font-headline tracking-widest text-xs md:text-sm text-on-surface">
            <div className="flex items-center gap-4 pb-1 overflow-hidden">
              ...
              <a href={`mailto:${contactEmail}`} className="hover:text-primary transition-colors block truncate w-full">
                {contactEmail}
              </a>
            </div>
        ...
        <div className="w-full lg:w-1/2 mt-8 lg:mt-0">
          {success ? (
            <div className="h-full flex flex-col justify-center bg-surface-container-low p-8 mx-auto md:p-12 border-l-4 border-primary">
              <h3 className="font-headline text-2xl md:text-3xl font-bold text-on-surface mb-4 uppercase tracking-tight">Transmission Successful</h3>
              <p className="font-body text-sm md:text-base text-on-surface-variant">Your message has been safely received. I will be in contact shortly.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-8 md:gap-10">
              
              <div className="flex flex-col relative w-full">
                <label htmlFor="name" className="font-headline text-[10px] md:text-xs tracking-widest uppercase text-outline mb-2">Identifier</label>
                <input 
                  type="text" 
                  id="name" 
                  name="name" 
              ...
              <div className="flex flex-col relative w-full">
                <label htmlFor="email" className="font-headline text-[10px] md:text-xs tracking-widest uppercase text-outline mb-2">Relay Node (Email)</label>
                <input 
                  type="email" 
                  id="email" 
                  name="email" 
              ...
              <div className="flex flex-col relative w-full">
                <label htmlFor="message" className="font-headline text-[10px] md:text-xs tracking-widest uppercase text-outline mb-2">Payload (Message)</label>
                <textarea 
                  id="message" 
                  name="message" 
                  required
