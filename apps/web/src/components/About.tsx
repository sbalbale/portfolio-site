...
export default function AboutSection({ data }: { data?: any }) {
  ...
  const primaryStatement = data?.primaryStatement || "I build high-performance embedded systems and resilient infrastructure, adhering to the principle that architectural clarity yields absolute scale.";
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:grid md:grid-cols-12 gap-12 md:gap-16">
        ...
        <div className="md:col-span-4 relative">
          <div className="md:sticky md:top-32">
            <h2 className="font-headline text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter uppercase text-on-surface leading-tight md:leading-none">
              {heading}
              <br />
              <span className="text-primary tracking-tighter">{subheading}</span>
            </h2>
          </div>
        </div>
        ...
        <div className="md:col-span-8 flex flex-col">
          <p className="font-headline text-2xl md:text-3xl lg:text-4xl font-light leading-snug md:leading-relaxed text-on-surface mb-8">
            {primaryStatement}
          </p>
          
          <div className="flex flex-col gap-6 w-full">
            {bodyParagraphs.map((paragraph: string, index: number) => (
              <p key={index} className="font-body text-base md:text-lg lg:text-xl text-on-surface-variant leading-relaxed">
                {paragraph}
              </p>
            ))}
          </div>
